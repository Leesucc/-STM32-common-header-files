...
#define SIZE sizeof(reg_jioashika)		//数组长度
...
void Time_Regulate(void)
{
  ...
  memset(&time, 0 , sizeof(time) );	/* 清空结构体 */
		...
		time.tm_year =UDP_year;
	 time.tm_mon=UDP_mon-1;
	time.tm_mday=UDP_mday;
	time.tm_hour=UDP_hour;
	 time.tm_min=UDP_min;
	time.tm_sec=UDP_sec;                                                                                               
...
void RTC_Init(u8 RTC_temp)
{
  ...
  if ((BKP_ReadBackupRegister(BKP_DR1) != 0xA5A5)||RTC_temp)
  {
...
void Time_get(void)
{
   ...
   time = Time_GetCalendarTime();
		...
		RTC_mon = time.tm_mon+1;
		RTC_mday = time.tm_mday;
